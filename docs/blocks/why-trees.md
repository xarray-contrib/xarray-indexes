...
```{tip}
**Ready to use trees with xarray?** See [NDPointIndex](ndpoint.md) for how to use KD-trees and Ball trees with xarray's indexing system for efficient nearest-neighbor lookups on real datasets.
```
...
Ready to use tree-based indexing with xarray? See [NDPointIndex](ndpoint.md) for how to integrate KD-trees and Ball trees with xarray's indexing system.
